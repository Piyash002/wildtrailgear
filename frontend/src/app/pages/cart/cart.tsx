...
const Cart = () => {
    ...
    return (
      ...
      <div className="lg:p-4 bg-slate-200 border border-slate-300">
      <h1 className=" text-xl font-bold mb-2">Cart Items</h1>
      <div className="  border border-slate-300 grid grid-cols-5 mb-2 p-1">         
      ...
      {cartItems.map(item => (
        <div key={item._id}  className="grid grid-cols-5   border border-slate-300 mb-3 items-center p-2">      
        ...
        // <div key={item._id}  className="grid grid-cols-4  bg-stone-300 p-6">
        //   <div className="  flex gap-x-1 col-span-2">
        //    <img src={item.image} alt="" className="w-8 h-8" />
        //    <h2 className=" overflow-x-hidden">{item.productName}</h2>
        //   </div>
        //   <div className="  col-span-1 text-center  ">
        //      <div className="text-center text-secondary text-lg font-body">৳{item.price}</div>
        //    <button className=" " onClick={() => dispatch(removeFromCart(item._id))}><MdDeleteOutline /></button>
        //   </div>

        //   <div className="   flex items-center col-span-1 ">
        //   <button className="bg-slate-100 p-2 rounded" onClick={() => dispatch(decreaseQuantity(item._id))}>-</button>
        
        //    <span className="px-2">{item.quantity}</span>
        //    <button className="bg-slate-100 p-2 rounded" onClick={() => dispatch(increaseQuantity(item._id))}>+</button>
        //   </div>
